...
- **Push must work for every non-pull-only remote.** Jira, Vikunja, and
  MS Todo are expected to round-trip. Notion stays pull-only by design.
...
### 1.2 Bidirectional sync for every non-pull-only remote

Today Jira / Vikunja / MS Todo drivers have read paths (with pagination)
and stub writes that raise `NotImplementedError`. The `ProtocolHandler`
swallows that exception and still reports `ok <ref>` to git. Result:
`git push` exits 0 and the user believes their edits propagated.
...
2. **Implement the writes (FEAT-02).** Full upsert + delete + ID-back
   mapping for Jira, Vikunja, MS Todo. Contract borrowed from
   `todo-harvest/src/sources/*.py`: each driver exposes `push(tasks)` and
   surfaces per-task `PushResult` (created / updated / skipped / failed).
...
1. **P0 correctness sweep.** BUG-01 → BUG-04 → BUG-05 → BUG-07 → SEC-01 → SEC-02. Add the tests in TEST-02 / TEST-03 alongside the bugs they uncover.
2. **Runtime honesty pass.** BUG-02, FEAT-04. This unblocks all "the push looked fine" user reports before writes exist.
3. **`tasks-init` ergonomics.** DX-04, DX-01.
4. **Write paths.** FEAT-01 (MSAL) → FEAT-02 (per-driver `push`) → FEAT-03 (mapping). Port the contract from `todo-harvest`.
5. **Org agenda parity.** FEAT-05, DOC-03 cleanup.
6. **Remaining polish.** BUG-06, BUG-08, BUG-10, BUG-11, DX-02, DX-03, TEST-01, DOC-01, DOC-02.

## 4. Out of scope (for now)

- Replacing the hand-written YAML parser with PyYAML. Spec forbids.
...
- Changing the Notion pull-only stance.
